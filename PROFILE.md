...
| ⬜ **Task 9's audio has never played on the phone.** Built, `check`-green, installed — but the airplane-mode test that *is* the done-when has not run, so "playable offline" is a claim. | Run it the next time the phone is free. Until then task 9 stays unticked. |
